...
void InstructionsDisplay(ALLEGRO_FONT* title, ALLEGRO_FONT* text, ALLEGRO_FONT* paragraph) {
    ...
    al_draw_text(title, UNITED_NATIONS_BLUE, _WINDOW_WIDTH / 2, 0.5 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_CENTER, "Description");
    ...
    DrawButton(_WINDOW_WIDTH / 5, 7.8 * _WINDOW_HEIGTH / 9, 2 * _WINDOW_WIDTH / 5,
        8.8 * _WINDOW_HEIGTH / 9, text, "REGRESAR");
    //Boton Origen
    DrawButton(3 * _WINDOW_WIDTH / 5, 7.8 * _WINDOW_HEIGTH / 9, 4 * _WINDOW_WIDTH / 5,
        8.8 * _WINDOW_HEIGTH / 9, text, "ORIGEN");
}
...
void MoveSelection_Instructions(int Button) {
    ...
    {
        case _MENU:
            al_draw_filled_rectangle(_WINDOW_WIDTH / 5, 7.8 * _WINDOW_HEIGTH / 9, 2 * _WINDOW_WIDTH / 5, 8.8 * _WINDOW_HEIGTH / 9, al_map_rgba_f(0.3, 0.3, 0.3, 0.3));
            break;
        case _NEXT:
            al_draw_filled_rectangle(3 * _WINDOW_WIDTH / 5, 7.8 * _WINDOW_HEIGTH / 9, 4 * _WINDOW_WIDTH / 5, 8.8 * _WINDOW_HEIGTH / 9, al_map_rgba_f(0.3, 0.3, 0.3, 0.3));
            break;
    }
}
...
void OriginDisplay(ALLEGRO_FONT* title, ALLEGRO_FONT* text, ALLEGRO_FONT* paragraph) {
    ...
    al_draw_text(paragraph, WHITE, _WINDOW_WIDTH / 9, 3.2 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_LEFT,
        "of pure gold. Day and night the priests take turns moving the discs, without deviating");
    al_draw_text(paragraph, WHITE, _WINDOW_WIDTH / 9, 3.7 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_LEFT,
        "from the fixed and immutable rules imposed by Brahma.");
    al_draw_text(paragraph, WHITE, _WINDOW_WIDTH / 9, 4.2 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_LEFT,
        "Each priest should not move more than one disc at a time, and only should place a disc on");
    al_draw_text(paragraph, WHITE, _WINDOW_WIDTH / 9, 4.7 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_LEFT,
        "a free stake, or on a larger disc.");
    al_draw_text(paragraph, WHITE, _WINDOW_WIDTH / 9, 5.2 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_LEFT,
        "When the priests move the entire column of discs to the third tower, the tower and the ");
    al_draw_text(paragraph, WHITE, _WINDOW_WIDTH / 9, 5.7 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_LEFT,
        "Brahmans will turn to dust and it will be the end of the world.");
    al_draw_text(paragraph, WHITE, _WINDOW_WIDTH / 9, 6.4 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_LEFT,
        "If the priests were able to perform one movement every second, what would be the time ");
    al_draw_text(paragraph, WHITE, _WINDOW_WIDTH / 9, 6.9 * _WINDOW_HEIGTH / 9, ALLEGRO_ALIGN_LEFT,
